...
object fp3 {
  ...
  def removeDupes1 [X] (xs:List[X]) : List[X] = {
    ...
    xs match{
		...
		case y::Nil => return List(y)
		case y::ys => if(y == ys.head) return removeDupes1(ys)
		else return y::removeDupes1(ys)	
	}
  }
  ...
  def removeDupes2 [X] (xs:List[X]) : List[(Int, X)] = {
    // TODO: Provide definition here.
    removeDupes2Helper(xs, 1)
  }

  def removeDupes2Helper [X](xs: List[X], z:Int) : List[(Int, X)] = {
   xs match {
		case Nil => return List()
		case y::Nil => List((1 , y))
		case y::ys => if(y == ys.head) return removeDupes2Helper(ys, z+1)
		else return (z, y)::removeDupes2Helper(ys, 1) 
  ...
  def splitAt [X] (n:Int, xs:List[X]) : (List[X], List[X]) = {
	...
	else if( n >= xs.length) return (xs, List())
	else
    splitAtHelper(n, xs, 0, List())
  }

  def splitAtHelper [X] (n:Int, xs:List[X], count:Int, tempList:List[X]) : (List[X], List[X]) = {
	xs match {
		case Nil => (List(), List())
		case y::ys => 
		if(n == count)
		return (tempList, y::ys)
		else
		return splitAtHelper(n, ys, count+1, tempList:::List(y) )
	}
  }
  ...
  def allDistinct (xs : List[Int]) : Boolean = {
    ...
    xs match {
		case Nil => true
		case y::Nil => true
		case y::ys => if (member(y, ys)) return false;
		else return allDistinct(ys)
	}
  }
}
